...
import time
import algorithm.DRL4Route.ActorCritic as ac
...
def get_reinforce_sample(pred_steps, label_steps, label_len, pad_value, rl_log_probs):
    pred = []
    label = []
    label_len_list = []
    rl_log_probs_list = []
...
def get_nonzeros(pred_steps, label_steps, label_len, pad_value):
    pred = []
    label = []
    label_len_list = []
    for i in range(pred_steps.size()[0]):
...
def get_model_function(model):
    ...
    model_dict[model] = (Actor.DeepRoute, Actor.save2file)
...
def calc_reward(sample, label, params):
    ...
    reward = params['r_0'] #设定初始reward - 第t步动作的距离差
...
class DRL4Route(object):
    def filter_sample(self, V_reach_mask, rl_log_prob, greedy_out, label, scores):
        #remove samples with label_len = 0
        V_reach_mask_list = []
        label_list = []
        rl_log_prob_list = []
        greedy_out_list = []
    ...
    def get_values(self,V_reach_mask, rl_log_prob, sample_out, label, state_values, params):
        V_reach_mask, rl_log_prob, sample_out, label, state_values = self.filter_sample(V_reach_mask.reshape(-1, params['max_task_num']), rl_log_prob, sample_out, label.reshape(-1, params['max_task_num']), state_values)
        r_steps = []
        state_value_samples = []
        rl_log_probs = []
    ...
    def run(self, params, DATASET, process_batch, test_model):
        ...
        if (params['pre_train'] == True) or (params['model'] == 'DRL4Route_REINFORCE'):
            ...
            params['model_path'] = '/home/zsxz/graph2route_release/data/dataset/logistics/sort_model/hidden_size-16.deeproute_scst_logistics1670220248.5822163.csv1670220248.582231'
        ...
        for epoch in range(params['num_epoch']):
            ...
            with tqdm(train_loader, total=len(train_loader), postfix=postfix) as t:
                ...
                val_result = test_model(model, val_loader, self.device, params['pad_value'], params, save2file, 'val')  # 对于验证集上，不需要写结果；
