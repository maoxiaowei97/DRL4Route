...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context, V_reach_mask, device, V, sample, type):
        ...
        if type == 'mle':
            ...
            return (torch.stack(outputs, 1), torch.stack(selections, 1), torch.stack(decoder_outputs, 1), torch.stack(test_states, 1), scores)
        else:
            log_probs = []
            for i in steps:
...
class DeepRoute(nn.Module):
    ...
    def model_file_name(self):
        ...
        file_name = f'{file_name}.DRL4Route{time.time()}'
        return file_name
...
def save2file(params):
    file_name = ws + f'/output/output_0131/{params["model"]}.csv'
    # 写表头
    head = [
        # data setting
