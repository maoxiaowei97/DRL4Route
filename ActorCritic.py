...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context, V_reach_mask, device, V, sample, type):
        """
        Args:
        ...
        """
        pad_value = embedded_inputs.size(0) - 1
        ...
        if type == 'mle':
            for i in steps:
                ...
                V_masked = V.reshape(V.shape[0] * V.shape[1], V.shape[2], V.shape[3]) * (~mask + 0).unsqueeze(2)
                states = torch.cat([V_masked.reshape(batch_size, -1), probs], axis = 1)
                ...
                # decoder_state = hidden[0].clone()
                # context_state = context.clone()
                # decoder_input_state = decoder_input.clone()
                decoder_outputs.append(torch.concat([hidden[0].clone(), (context.permute(1, 0, 2).contiguous() * ((~mask) + 0).unsqueeze(2)).clone().reshape(batch_size, -1)], axis=1))
                ...
                scores.append(score)
                # selections_T[:, i]
                # selections_T.append(selections + [pad_value] * (steps - i - 1))

            return (torch.stack(outputs, 1), torch.stack(selections, 1), torch.stack(decoder_outputs, 1), torch.stack(test_states, 1), torch.stack(scores, 1).squeeze(2))
        else:
            log_probs = []
            for i in steps:
                hidden, log_p, probs, mask, score = self.recurrence(decoder_input, hidden, mask, idxs, i, context)
                # select the next inputs for the decoder [batch_size x hidden_dim]
                idxs, log_prob = self.decode(
                    probs,
                    mask,
            ...
            return (torch.stack(outputs, 1), torch.stack(selections, 1), torch.stack(log_probs, dim=1), torch.stack(scores, 1).squeeze(2))
...
class DeepRoute(nn.Module):
    ...
    def enc_sort_emb(self, sort_emb, sort_len, batch_size, max_seq_len):
        ...
        """
        attn_mask = self.get_transformer_attn_mask(max_seq_len, batch_size, sort_len)
    ...
    def forward(self, V, V_reach_mask, V_len, sample, type):
        '''

        V: (B, N, 3), the basic feature of a task, incluing lat, lng, promise time
        V_reach_mask: (B, T, N), reachable nodes at each step
        label: (B, T, N), label route at each step
        label_len: (B, T), label length at each step
        V_pt: (B, N), promise time of each task
        E_sd: (B, N, N), edudian distance of all tasks
        V_ft: (B, N), finish time of each task
        start_idx: (B, T), start node index at each step
        V_dt: (B, N), dispatch time of each task
        V_type: (B, N), type (pick-up or delivery node) of each node
        V_num: (B, T, N), numer of unfinished nearby neighbor nodes of each task
        E_mask: (B, T, N, N), edge mask at each time step
        V_dispatch_mask: (B, T, N, N), dispatched nodes at each step
        V_len: (B, T, 1), max valid len of each sample, same for each step.
        '''
        ...
        decoder_input, inputs, dec_init_state, enc_h = \
            self.enc_sort_emb(sort_x_emb, V_len.reshape(-1), B * T, N)

        if type == 'mle':
            (pointer_log_scores, pointer_argmaxs, decoder_outputs, test_states, greedy_values) = self.decoder \
                (decoder_input, inputs, dec_init_state, enc_h,  V_reach_mask.reshape(-1, N), V_reach_mask.device, V, sample, type)
            pointer_scores = pointer_log_scores.exp()  # (B * T, N, N)
            # baseline = self.critic()

            return pointer_scores, pointer_argmaxs, decoder_outputs, test_states, greedy_values
    ...
    def model_file_name(self):
        ...
        file_name = f'{file_name}.a2c_logistics{time.time()}.csv'
        return file_name
...
def save2file(params):
    ...
    file_name = ws + f'/output/output_131/{params["model"]}.csv'
    # 写表头
    head = [
        # data setting
